...
| `login_id`      | string    | [required] [unique]         | Catechist → `CAT-<member_id>`; Student → `STD-<student_code>`                |
...
> **Auto-creation:** an `Account` row is inserted automatically when a `Catechist` or `Student` is created. Default password = `loginId`. Users should change on first login.
