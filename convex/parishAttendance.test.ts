...
describe('getStudentAttendanceReport backend function', () => {
  async function setupTest() {
    const t = convexTest(schema, modules)
    ...
    const ids = await t.run(async (ctx) => {
      ...
      // Catechism session in active academic year — should be filtered out (wrong sessionType)
  ...
  test('returns mass and extracurricular records for the active academic year sorted by most recent first', async () => {
    ...
    // Sorted by deviceQueuedAt descending.
    expect(report[0].deviceQueuedAt).toBeGreaterThan(report[1].deviceQueuedAt)
  })
...
describe('listMyParishAttendance backend function', () => {
  // Shares the exact fixture shape as getStudentAttendanceReport above since
  // both now call the same resolveParishAttendanceForStudent(ctx, studentId)
  // helper — listMyParishAttendance just passes args.requesterId (a
  // students._id) straight through as the studentId.
  async function setupTest() {
    const t = convexTest(schema, modules)
